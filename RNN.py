...
def updateWord2Vec(_origin,_new,_originModel):
    ...
    #print(_wModel.wv.most_similar('哭', topn=10))
    #print(_wModel.wv['茶'])
...
class CoreNet(nn.Module):
    def __init__(self):
        ...
        self.n_layers = n_layers = 2 # RNN的层数
        self.hidden_dim = hidden_dim = 512 # 隐状态的维度
        
        drop_prob = 0.5 # dropout层概率

        self.RNN = nn.RNN(vector_size_, # 输入的维度
        ...
        # 全连接线性层
        self.fc = nn.Linear(in_features=hidden_dim, # 将RNN的输出作为线性层的输入
                            out_features=1 # 输出0或
                            ) 
        self.sigmoid = nn.Sigmoid() #过一下sigmoid
...
def train(_model,_train_loader,epochs = _epoch ,counter = 0,print_every = 10):
    ...
    for i in range(epochs):
        h = _model.init_hidden(batch_size) # 初始化第一个Hidden_state
